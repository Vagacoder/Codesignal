#
# * Core 134. New Year Celebrations

# * You're a pretty busy billionaire, and you often fly your personal Private Jet 
# to remote places. Usually travel doesn't bother you, but this time you are 
# unlucky: it's New Year's Eve, and since you have to fly halfway around the world, 
...
#     The take off time in the 24-hour format "HH:MM". It is guaranteed that the 
#       given time is valid. The "00:00" time corresponds to the midnight of 31st 
#       of December / 1st of January.
...
#     A strictly increasing array of integers. minutes[i] stands for the number 
#       of minutes from the take off to crossing the ith time zone border.
...
def newYearCelebrations(takeOffTime, minutes):
    ...
    t0 = dt.datetime.strptime(takeOffTime, '%H:%M')
    t1 = dt.datetime.strptime(takeOffTime, '%H:%M')
    # print(t0)

    if t0 == dt.datetime(1900, 1, 1, 0, 0, 0):
        t0 = dt.datetime(1900, 1, 2, 0, 0, 0)
        t1 = dt.datetime(1900, 1, 2, 0, 0, 0)
        
    
    newYear = dt.datetime(1900, 1, 2, 0, 0, 0)
    # print(newYear)

    count = 0

    for m in minutes:
        deltaM = dt.timedelta(minutes=m)
        t2 = t0 + deltaM
        # print(t2)
        # print(newYear - t1)
        # print(t2 - newYear)
        # print((t2 - newYear).days > 0)
        if (newYear - t1).days >= 0 and (t2 - newYear).days >= 0:
            count += 1
        t1 = t2
        newYear = newYear + dt.timedelta(minutes=60)

    if (newYear - t1).days >= 0:
        count += 1
