#
# * Core 41. Replace Middle
# * Easy

# * We define the middle of the array arr as follows:

#     if arr contains an odd number of elements, its middle is the element whose 
#       index number is the same when counting from the beginning of the array 
#       and from its end;
#     if arr contains an even number of elements, its middle is the sum of the 
#       two elements whose index numbers when counting from the beginning and 
#       from the end of the array differ by one.
...
#     arr with its middle replaced by a single element, or the initial array if 
#       the middle consisted of a single element to begin with.

#%%
# * Solution 1
def replaceMiddle1(arr):
    n = len(arr)
    if n%2 == 0:
        middle = arr[n//2] + arr[n//2-1]
        return arr[:n//2-1] + [middle] + arr[n//2+1:]
    else:
        return arr


a1 = [7, 2, 2, 5, 10, 7]
r1 = replaceMiddle1(a1)
print(r1)
