...
# = 47 / 9 = 5.2222 = 5. The other three integers are obtained the same way, then 
...
def boxBlur(image: list)-> list:
    ...
    def blurFilter(i: int, j:int)-> int:
        return int((image[i-1][j-1] +
                image[i-1][j] +
                image[i-1][j+1] +
                image[i][j-1] +
                image[i][j] +
